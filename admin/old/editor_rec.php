...
if (isset($_GET['edit_id'])) {
	$rec_id = $_GET['edit_id'];
	$rec_class = $_GET['rec_class'];
	
	if($rec_class=="talent"){
		$sql = 'SELECT * FROM talent_rec
			WHERE rec_id='.$rec_id;
	}else if($rec_class=="society"){
		$sql = 'SELECT * FROM society_rec
			WHERE rec_id='.$rec_id;
	}else{
	...
	while($result = mysql_fetch_assoc($retval)) {
			$project['rec_id'] = $result['rec_id'];
			$project['rec_title'] = $result['rec_title'];
			$project['rec_jobs'] = $result['rec_jobs'];
			$project['rec_demand'] = $result['rec_demand'];
			$project['rec_time'] = $result['rec_time'];
		}
...
//    传输sql语句
    ...
    if (!$retval) {
		...
		echo "<script>alert('招聘信息发布成功！');location.href='main.php';</script>";   
	}
                            ...
                            <div class="panel-body">
                                <div class="row">
                                    <div class="col-sm-7">
                                        ...
                                        <form class="form-horizontal" id="addREC" method="post" action="<?php $_PHP_SELF ?>" >

                                            <div class="form-group">
                                                <label for="recTitle" class="col-sm-2 control-label">职位名称：</label>
                                                <div class="col-sm-10">
													<?php
														echo "<input name='recTitle' class='form-control' type='text' id='recTitle' value=".$project['rec_title'] .">";
													?>
                                                </div>
                                            </div>
                                            <div class="form-group">
													  ...
													  <?php
														if($rec_class=="talent"){
															echo " <option value=''>--请选择--</option>
																   <option value='talent' selected='selected'>人才招聘</option>
															...
															echo " <option value=''>--请选择--</option>
																   ...
																   <option value='society' selected='selected'>社会招聘</option>
																   <option value='school'>校园招聘</option>";
														}else{
															echo " <option value=''>--请选择--</option>
																   <option value='talent'>人才招聘</option>
																   <option value='society'>社会招聘</option>
																   <option value='school' selected='selected'>校园招聘</option>";
														}
													  ?>
													</select>
                                                </div>
                                            ...
                                            <div class="form-group">
                                                ...
                                                <div class="col-sm-10">
													<?php
														echo "<script id='recJobs' name='recJobs' type='text/plain'>".$project['rec_jobs']."</script>"
													?> 
                                                </div>
                                            </div>
											 ...
											 <div class="form-group">
                                                ...
                                                <div class="col-sm-10">
													<?php
														echo "<script id='recDemand' name='recDemand' type='text/plain'>".$project['rec_demand']."</script>"
													?>
                                                </div>
                                            </div>
